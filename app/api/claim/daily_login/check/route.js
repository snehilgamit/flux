...
export async function POST(req) {
    ...
    const { ok, get_user:user, message } = await session(data)
    if (!ok) {
    ...
    const temporary = new Date(get_user.daily_login.timestamp)
    temporary.setDate(temporary.getDate() + 1)
    temporary.setHours(0, 0, 0, 0)
    
    const timestamp = temporary.getTime()
